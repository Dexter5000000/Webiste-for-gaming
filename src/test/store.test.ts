...
import { useAppStore, createEmptyProject, createInitialTransport, createInitialSelection, createInitialGrid } from '../state/store';
...
describe('Track CRUD Operations', () => {
  it('should add different types of tracks', () => {
    ...
    expect(useAppStore.getState().project.tracks).toHaveLength(initialTrackCount + 1);
    ...
    expect(useAppStore.getState().project.tracks).toHaveLength(initialTrackCount + 2);
    const midiTrack = useAppStore.getState().project.tracks[initialTrackCount + 1];
    ...
    const masterTrack = useAppStore.getState().project.tracks.find(t => t.type === TrackType.MASTER);
  ...
  it('should rename tracks', () => {
    ...
    expect(useAppStore.getState().project.tracks[0].name).toBe('New Track Name');
  });
  ...
  it('should duplicate tracks with clips and effects', () => {
    ...
    const originalEffectCount = useAppStore.getState().project.effectChains[0].effects.length;
    ...
    expect(state.project.effectChains[1].effects).toHaveLength(originalEffectCount);
  });
...
describe('Clip CRUD Operations', () => {
  ...
  it('should add audio clips', () => {
    ...
    expect(clips[0].warping.enabled).toBe(true);
  });
...
describe('Effect Operations', () => {
  ...
  it('should remove effects', () => {
    ...
    expect(useAppStore.getState().project.effectChains[0].effects).toHaveLength(0);
  });
  ...
  it('should set effect parameters', () => {
    ...
    const updatedEffect = useAppStore.getState().project.effectChains[0].effects[0];
...
describe('Grid Settings', () => {
  ...
  it('should toggle grid settings', () => {
    ...
    expect(useAppStore.getState().grid.showTrackNumbers).toBe(!initialGridState);
  });
});
...
describe('History Operations', () => {
  it('should save and restore from history', () => {
    ...
    expect(useAppStore.getState().project.tracks).toHaveLength(initialProject.tracks.length);
  });
